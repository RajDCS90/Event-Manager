...
const GrievanceTable = ({ skipInitialFetch = false }) => {
  ...
  }, [statusFilter, mandalFilter, areaFilter, villageFilter, boothFilter, dateRange, skipInitialFetch])
  ...
  const handleMandalFilterChange = (e) => {
    ...
    // Update area options based on selected mandal
    if (value) {
  ...
  const handleAreaFilterChange = (e) => {
    ...
    // Update village options based on selected area
    if (value) {
  ...
  const handleVillageFilterChange = (e) => {
    ...
    // Update booth options based on selected village
    if (value) {
  ...
  const handleBoothFilterChange = (e) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div>
            ...
            >
              <option value="">All Mandals</option>
              {mandals.map((mandal) => (
                <option key={mandal.mandalName} value={mandal.mandalName}>
                  {mandal.mandalName}
                </option>
              ))}
            </select>
          ...
          <div>
            ...
            <select
              value={boothFilter}
              onChange={handleBoothFilterChange}
              className="w-full p-2 border border-gray-300 rounded-md focus:ring-blue-500 focus:border-blue-500"
      ...
      {error && <div className="mx-6 mt-4 p-3 bg-red-50 text-red-700 rounded-lg border border-red-200">{error}</div>}
              ...
              {filteredGrievances.length === 0 ? (
                ...
                filteredGrievances.map((grievance) => (
                  <tr key={grievance._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      {formatDate(grievance.programDate)} at {formatTime(grievance.startTime)} -{" "}
                      {formatTime(grievance.endTime)}
                    </td>
                    <td className="px-6 py-4">
                      <StatusBadge status={grievance.status} />
